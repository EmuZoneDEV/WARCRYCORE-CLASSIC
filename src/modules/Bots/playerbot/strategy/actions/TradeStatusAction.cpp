...
bool TradeStatusAction::Execute(Event event)
{
    ...
    if (trader != master)
    {
        bot->Whisper("Ich habe gerade echt keine Zeit", LANG_UNIVERSAL, trader->GetObjectGuid());
    }
...
bool TradeStatusAction::CheckTrade()
{
    ...
    if (playerMoney >= botMoney)
    {
        switch (urand(0, 4)) {
        case 0:
            ai->TellMaster("Es war mir eine Freude mit dir zu Handeln");
            break;
        case 1:
            ai->TellMaster("Fairer Handel");
            break;
        case 2:
            ai->TellMaster("Danke");
            break;
        case 3:
            ai->TellMaster("Weg mit dir");
            break;
        }
        return true;
    }
    ...
    ostringstream out;
    out << "Ich will " << chat->formatMoney(botMoney - playerMoney) << " dafur";
    ai->TellMaster(out);
    return false;
}
